...
export default function Questionnaire({
  ...
  const hasExistingResponses =
    ...
    existingSubmission.responses.length > 0;
  ...
  useEffect(() => {
    if (hasExistingResponses && !isEditing) {
      ...
      existingSubmission.responses.forEach((response: any) => {
        initialResponses[response.question] =
  ...
  if (!questionnaire) {
    return (
      <div className="text-red-500">
        No questionnaire found for this process
      </div>
    );
  }
  ...
  const handleInputChange = (
    ...
    setResponses((prev) => ({
      ...prev,
      [questionId]:
        type === "text" ? { text_response: value } : { selected_choice: value },
    }));
  };
      ...
      const responseData = Object.entries(responses).map(
        ([questionId, response]) => ({
          ...
          ...(response.text_response !== undefined
            ? { text_response: response.text_response }
            : {}),
          ...(response.selected_choice !== undefined
            ? { selected_choice: response.selected_choice }
            : {}),
        })
      );
      ...
      if (result && !result.error) {
        ...
        if (result?.status === 403) {
          setError(
            `Permission denied: ${
              result.error ||
              "You do not have permission to submit this questionnaire"
  ...
  const getExistingResponse = (questionId: number) => {
    ...
    return existingSubmission.responses.find(
  ...
  const getRemainingQuestions = () => {
    if (!hasExistingResponses) return questionnaire.questions;

    const answeredQuestionIds = existingSubmission.responses.map(
      (response: any) => response.question
    );
    const remainingQuestions = questionnaire.questions.filter(
      (question: any) => !answeredQuestionIds.includes(question.id)
    );
        ...
        <div>
          ...
          <div className="mb-6">
            <h3 className="text-lg font-medium mb-4">
              Your Remaining Responses:
            </h3>
            {remainingQuestions.length > 0 ? (
              <div className="space-y-3">
                {remainingQuestions.map((question: any) => (
                  <div
                    key={question.id}
                    ...
                    {question.choices.map((choice: any) => (
                      <div key={choice.id} className="flex items-center">
                        <input
                          ...
                          onChange={() =>
                            handleInputChange(question.id, choice.id, "choice")
                          }
                          defaultChecked={
