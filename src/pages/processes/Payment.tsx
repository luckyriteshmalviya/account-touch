...
import { uploadDocumentService, updatePaymentStatusService, requestPaymentService } from "../../services/restApi/task";
...
export default function Payment({ process, onComplete, onPrevious, userRole, refreshProcess }: PaymentProps) {
    ...
    const minimumFees = process?.process_template_detail?.fees || 
    ...
    const isAdminOrChecker = userRole === 'super-admin' || userRole === 'admin' || userRole === 'checker';
    ...
    const handleFileChange = async (documentType: any, event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const result = await uploadDocumentService(process.id, documentTypeIdentifier, file);
    ...
    const handleStatusUpdate = async (newStatus: 'PENDING' | 'IN_PROGRESS' | 'COMPLETED' | 'REJECTED' | 'CANCELLED') => {
        setIsLoading(true);
                ...
                <div className="mb-8">
                    ...
                    <div className="space-y-4">
                        {requiredDocuments.map((document: any) => {
                            ...
                            return (
                                <div key={documentId} className={`border rounded-lg p-4 ${
