...
export default function ActiveTasksList() {
  ...
  const statusFilters: Record<string, string> = {
    ...
    steps_procedure: "in_progress,pending",
    document_preparation: "approved",
    default: "in_progress,pending,waiting_for_approval,approved",
  };

  const handleTaskClick = (processType: string) => {
    const status = statusFilters[processType] || statusFilters.default;
  ...
  return (
    <div className="border border-gray-200 bg-white p-5 rounded-2xl shadow-lg text-sm text-gray-800 w-full">
      <div
        ...
        onClick={() => navigate(`/task-list?status=${statusFilters.default}`)}
      >
      ...
      {loading ? (
        ...
        <ul className="space-y-3 mt-2">
          {processCounts.map((item: any, idx: number) => {
            const processType = item.process_template__process_type;
            const displayName = processType.replace(/_/g, " ");
            return (
              <li
                key={idx}
                onClick={() => handleTaskClick(processType)}
              ...
              >
                <span className="capitalize text-gray-700">{displayName}</span>
