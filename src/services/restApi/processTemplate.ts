...
export const getProcessTemplatListService = async (params: {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(
        `Error fetching ProcessTemplat list: ${res.status} ${res.statusText}`
      );
      return null;
    }
...
export const getProcessTemplatDetailsService = async (id: string) => {
  try {
    const token = getAccessToken();
    const res = await fetch(
      `https://api.accountouch.com/api/tasks/process-templates/${id}/`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export const addProcessTemplatService = async (payload: any) => {
  const token = getAccessToken();
  const res = await fetch(
    "https://api.accountouch.com/api/tasks/process-templates/",
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const deleteProcessTemplatService = async (processTemplatId: number) => {
  try {
    const token = getAccessToken();
    const res = await fetch(
      `https://api.accountouch.com/api/tasks/process-templates/${processTemplatId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
