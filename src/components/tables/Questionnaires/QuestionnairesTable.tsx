...
export default function QuestionnairesTable() {
  ...
  const handleDelete = async () => {
    if (deleteId !== null) {
      ...
      if (success) {
        setQuestionnaires(prev => prev.filter(ques => ques.id !== deleteId));
        Swal.fire({
          icon: 'success',
          title: 'Deleted!',
          ...
          <div className="min-w-[850px]">
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  {["Title", "Id", "IS Active", "Created At", "Number Of Question", "Description", "Actions"].map(
                    (header) => (
                      <TableCell
                        key={header}
                        isHeader
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {questionnaires.map((ques) => (
                  <TableRow key={ques.id} className="text-center">
                    <TableCell className="px-4 py-4 text-start bold text-[#417893]">{ques.title}</TableCell>
                    <TableCell className="px-4 py-4 text-start">{ques.id}</TableCell>
                    <TableCell className="px-4 py-4 text-start">{ques.is_active ? (
                    ...
                    <TableCell className="px-4 py-4 text-start">{ques.questions?.length || "-"}</TableCell>
                    <TableCell className="px-4 py-4 text-start">{ques.description || "-"}</TableCell>
                    <TableCell className="flex items-center gap-3 px-4 py-3">
                      <Eye
                        className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
                        onClick={() => navigate(`/questionnaires/view/${ques.id}`)}
                      />
                      <Edit
                        className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
                        onClick={() => navigate(`/manage-questionnaires/${ques.id}`)}
                      />
                      <Trash
