...
export default function ProcessTemplatTable() {
    ...
    const handleDelete = async () => {
        if (deleteId !== null) {
            ...
            if (success) {
                setProcessTemplat(prev => prev.filter(proc => proc.id !== deleteId));
                Swal.fire({
                    icon: 'success',
                    title: 'Deleted!',
                    ...
                    <div className="min-w-[850px]">
                        <Table>
                            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                                <TableRow>
                                    {["Title", "Process Type", "Status", "Is Active", "Created At", "Required Document", "Action"].map(
                                        (header) => (
                                            <TableCell
                                                key={header}
                                                isHeader
                                ...
                                {processTemplat.map((proc) => {
                                    ...
                                    return (
                                        <TableRow key={proc.id} className="text-center">
                                            <TableCell className="px-4 py-4 text-start bold text-[#417893]">{proc.title}</TableCell>
                                            <TableCell className="px-4 py-4 text-start">{
                                                proc.process_type ? proc.process_type.replace(/_/g, " ").replace(/^\w/, (c) => c.toUpperCase())
                                                    : "-"}</TableCell>
                                            <TableCell className="px-4 py-4 text-start">{proc.status || "-"}</TableCell>
                                            <TableCell className="px-4 py-4 text-start">
                                            ...
                                            </TableCell> <TableCell className="px-4 py-4 text-start">{
                                                proc?.required_documents?.[0]?.created_at ?
                                                    new Date(proc?.required_documents?.[0]?.created_at)
                                                        ...
                                                        .replace("PM", "p.m.")
                                                    : "-"}</TableCell>
                                            <TableCell className="px-4 py-4 text-start">{doc?.name || "-"}</TableCell>
                                            <TableCell className="flex items-center gap-3 px-4 py-3">
                                                <Eye
                                                    className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
                                                    onClick={() => navigate(`/process-templates/view/${proc.id}`)}
                                                />
                                                <Edit
                                                    className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
                                                    onClick={() => navigate(`/manage-process-templates/${proc.id}`)}
                                                />
                                                <Trash
