...
} from "../../../services/restApi/documentType";
          ...
          <div className="min-w-[850px]">
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  {["#", "Name", "Is Active", "Description", "Actions"].map((header) => (
                    <TableCell
                      key={header}
                      isHeader
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {documentTypes.map((doc) => (
                  <TableRow key={doc.id} className="text-center">
                    <TableCell className="px-4 py-4 text-start">{doc.id}</TableCell>
                    ...
                    <TableCell className="px-4 py-4 text-start">{doc.description || "-"}</TableCell>
                    <TableCell className="flex items-center gap-3 px-4 py-3">
                      <Eye
                        className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
                        onClick={() => navigate(`/document-type/view/${doc.id}`)}
                      />
                      <Edit
                        className="w-5 h-5 text-blue-600 hover:text-blue-800 cursor-pointer"
